...
- 일괄 추출 중 구독 사용량 한도에 걸려 양식 16의 마지막 15장이 실패(오류 메시지 없음) → 한도 초기화 후 이어 하기로 100장 완료. 실제 운영이라면 외부 모델 한도·장애가 곧 업무 중단이므로 엔진 A 대체 경로가 필요하다는 근거.
- 금액 칸(가입금액·타사가입금액)은 숫자(+'만' 단위)만, 이메일은 대소문자 무시로 비교하도록 지표 수정 (두 엔진 동일 적용). 쓰는 사람마다 '원'·쉼표 표기가 달라 업무상 차이가 없는 불일치였음.
- 발견: 엔진 B는 드문 성씨를 흔한 성씨로 바꿔 읽는 경향(곡→곽, 뢰→최, 묘→표). 언어모델 사전지식 편향 → 위험평가(H) 신뢰성·편향성 항목의 근거.
- `st.cache_resource` 연결이 DB 경로를 무시하고 재사용되던 문제 → 경로별 캐시로 수정.
