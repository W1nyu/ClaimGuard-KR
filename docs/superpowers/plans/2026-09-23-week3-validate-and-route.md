...
| R09 | 16 | 직업 KSCO 정확 일치 (부분·유사는 후보 제시) | 담당자검토 |
...
각 Task의 전체 코드는 해당 커밋의 파일(`src/reference.py`, `src/validate.py`, `src/route.py`, `scripts/run_decision_eval.py`와 테스트)을 기준으로 한다.

## 구현 중 변경

- R09: 부분·유사 일치가 엉뚱한 직업을 고름(검투사→검사, 마케터→텔레마케터) → 정확 일치만 통과, 나머지는 후보와 함께 담당자검토.
- 은행: "국민은행"↔"KB국민" 인정, "마을"↔"새마을금고" 같은 중간 포함은 제외(공식 이름이 입력으로 끝날 때만 일치).
